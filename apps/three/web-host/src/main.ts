...
type RemoteWidgetModule = {
  mountThree: (canvas: HTMLCanvasElement) => {
    renderer?: { getSize: (target: { width: number; height: number }) => { width: number; height: number } };
...
const WIDGET_PORT = Number(__WIDGET_PORT__ || 5201);
...
async function connectOnce() {
  try {
    ...
    const globalMount =
      (globalThis as unknown as { __3D_WIDGET__?: { mountThree?: RemoteWidgetModule["mountThree"] } }).__3D_WIDGET__
        ?.mountThree;
    const mount = typeof mod.mountThree === "function" ? mod.mountThree : globalMount;
    if (typeof mount !== "function") {
      throw new Error(
        `mountThree를 찾을 수 없습니다. Three.js embed가 ESM export를 제공하지 않을 수 있습니다. (REMOTE_EMBED_URL=${REMOTE_EMBED_URL})`,
      );
    }
    ...
    debugIntervalId = window.setInterval(() => {
      ...
      if (Math.floor(rect.width) <= 0 || Math.floor(rect.height) <= 0 || cw <= 0 || ch <= 0) {
        setStatus(
          `캔버스 사이즈가 0입니다.\n` +
