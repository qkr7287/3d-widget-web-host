...
## 운영에서 “가져오는 구조”는 그대로, “가져오는 위치/형태”만 바뀜

운영에서도 핵심 구조는 동일합니다.

- Host가 원격 Widget을 `import()` 해서 실행한다 (구조 동일)
- 단, 개발과 달리 운영에서는 **소스(TS)** 가 아니라 **빌드 산출물(JS)** 만 가져옵니다.

정리:

- 개발(dev): `http://localhost:5174/src/embed.ts` (소스 엔트리, dev server가 변환해서 제공)
- 운영(prod): `https://cdn.example.com/widget/embed.js` (빌드 엔트리, dist 정적 파일만 제공)

## 운영에서는 dist 밖의 파일을 “직접” 가져오지 않는다

운영 환경에서 브라우저가 접근하는 것은 오직 정적 서버/CDN에 배포된 결과물입니다.
즉, 배포 후에는 **`dist/`에 포함되어 공개된 파일들(embed.js, assets/*)만** 네트워크로 로드됩니다.
